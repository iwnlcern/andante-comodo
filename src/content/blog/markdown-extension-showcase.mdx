...
draft: false
epistemic: "A living demo — this post tracks the template's markdown pipeline as it grows."
ai:
  level: 3
  tools: ["Claude"]
  note: "Demo values — set your own disclosure per post."
---
...
The metadata line under the title is mostly automatic: publish date, an "Updated" date (from `updated:` in frontmatter when set, otherwise the file's git history), reading time, and a word-count popover.

Two opt-in frontmatter fields complete the apparatus, and this post now uses both:

- `epistemic:` — a one-line status note, rendered as the banner above this post's intro. Use it to tell readers how settled the writing is ("a living document", "notes from one rehearsal", …).
- `ai:` — an AI-involvement disclosure on a 0–5 rubric (`level`, plus optional `tools` and `note`), rendered as the small signal-strength indicator in the metadata line. Hover it for the level name and description; `level: 0` renders empty bars — the Adagietto demo post shows that state. If you set `meta.aiPolicyHref` in `src/config/site.mjs`, the indicator links to your own AI-use policy page.

Both fields work on `blog` posts and `pages`; the field reference lives in `docs/README.md`.
